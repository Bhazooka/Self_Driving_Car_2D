...
class Visualizer:
    ...
    def plot_progress(self):
        ...
        # Scatter plot for each episode's score
        ax.scatter(self.episodes, self.scores, color='orange', alpha=0.5, s=18, label='Episode scores (scatter)')

        # Plot raw data with reduced opacity (line)
        ax.plot(self.episodes, self.scores, color='lightblue', alpha=0.3, label='Raw scores (line)')
        ...
        stats_text += f"Avg Score: {np.mean(self.scores[-200:]):.2f}"
